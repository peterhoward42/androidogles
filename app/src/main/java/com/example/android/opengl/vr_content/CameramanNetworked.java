 ...
 * the public injectedPosition field from time to time. The camera points always at the centre
 * of the scene. The injectedPosition field is NOT thread safe.
 */
public class CameramanNetworked implements Cameraman {

    private final static float HEIGHT_FACTOR = 1.1f;
    final Sphere mSphere;
    public XYZf injectedPosition;

    public CameramanNetworked(final Sphere sceneSphere) {
        this.mSphere = sceneSphere;
        this.injectedPosition = new XYZf(0, 0, 0);
    }
    ...
    private XYZf evaluatePosition() {
        final float eastings = -30.0f + this.injectedPosition.X() * 60.0f;
